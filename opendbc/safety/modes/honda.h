...
static safety_config honda_nidec_init(uint16_t param) {
  ...
  // Stock longitudinal: openpilot does not send gas/brake/ACC_HUD; it only spams cruise
  // buttons (0x296 SCM_BUTTONS on the Odyssey, 0x1A6 on some other Nidecs) to cancel/resume
  // the factory ACC, which actuates longitudinal via the forwarded camera brake command.
...
static bool honda_nidec_fwd_hook(int bus_num, int addr) {
  ...
  if (bus_num == 2) {
    // forwarded if stock AEB is active, or always when the factory ACC owns longitudinal
